...
var Ratio = (function () {
    ...
    Ratio.simplify2 = function(obj){
        ...
        factor = 1;
        if (arr.length) {
            if (arr[1].length / arr[2].length < 3) {
                top = Number(arr.join('')) - Number(arr[0] + String(arr[1]));
                bottom = Math.pow(10, arr[1].length) * (Math.pow(10, arr[2].length) - 1);
            } else {
                var x = Ratio.getApprox(top / bottom);
                top = x[0];
                bottom = x[1];
            }
        }
        if (String(obj.d).length < String(bottom).length) {
            top = obj._n;
            bottom = obj._d;
        } else {
            factor = Ratio.gcd(top, bottom);
